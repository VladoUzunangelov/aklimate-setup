...
LOAD_FULL_FEATURE_IMPORTANCE = FALSE
...
main <- function(argv) {
  ...
  data_dir <- "./data"
  sample_data_file <- paste0(data_dir, "/", "reduced_combined_matrix.tsv")

  output_dir <- "./models"
  feature_list_file <- paste0(output_dir, "/", "junkle_fully_trained_model_feature_importance.tab")
  model_output_file <- paste0("rf_reduced_model.RData")
  reduced_model_feature_importance_file <- paste0("rf_reduced_model_feature_importance.tsv")

  p_store_dir <- "./p_store_files"
  ...
  if (LOAD_SAMPLE_DATA_MATRIX) {
    ...
    found_meth_mapper_file <- TRUE %in% (list.files(path = p_store_dir) == saved_meth_mapper_file)

    saved_meth_mapper_file <- paste0(p_store_dir, "/", saved_meth_mapper_file)
  ...
  if (LOAD_FULL_FEATURE_IMPORTANCE) {
    ...
    message("skip loading full feature importance file")
  }
  ...
  if (TRUE %in% (list.files(path = output_dir) == model_output_file)) {
    ...
    # dat feature matrix has already been reduced to selected features
    k.adj <- dim(dat)[2]
    message(paste0("number of features, k.adj: ", k.adj))

    idx.train <- rownames(dat)
    message(paste0("length(idx.train): ", length(idx.train)))

    training_data <- cbind(data.frame(labels = labels[idx.train]), dat)
  ...
  colnames <- paste0("featureID\tfeature_importance_score_for_reduced_rf_model")
