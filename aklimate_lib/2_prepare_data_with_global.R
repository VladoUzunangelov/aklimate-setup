...
if (QUANTIZE_NUMERIC_DATA) {
  ...
  dat <- foreach(datatype_str = iter(mir[!grepl("MUTA|CNVR", nomir)]), .combine = cbind) %do%
    {
...
if (USE_GLOBAL==TRUE) {
	global_matrix <- dat[sampleIDs, grepl(paste0("GLOBAL:"), colnames(dat))]
	global_feature_list <- colnames(global_matrix)
}

message(paste0("global_features: ", length(global_feature_list)))
message(global_feature_list)
...
worker.f <- function(tasks) {
  ...
  res <- foreach(i = iter(tasks)) %do% {
    ...
    # classification_type <- "multiclass"
      ...
      #junkle_always_add <- sprintf("N:MIRSCORE::FAM_%s:", 0:29)
      junkle_always_add <- global_feature_list
