...
# outputfile <- paste0("/scratch/for_gchavez/aklimate_results", "sub_", tt, "-plot.pdf")
...
file_name_list <- list.files(model_dir, pattern = paste0("_cutoff_",reduced_model_cutoff,"_rf_reduced_model_stats.RData"), full.names = FALSE)
...
sub_cohort_names = c()
dat = read.table(subtype_name_mapping_file,
...
text_main <- paste0("Balanced accuracy by subtype\nfor ",
  tt,"\n(",reduced_model_cutoff," features)")
# yrange <- c(-0.35,1.1)
yrange <- c(0,1.06)
x = barplot(colMeans(crossval_bal_acc), main = text_main, xlab = "Subtype", ylab = "Balanced Accuracy", ylim = yrange, col = colors, border = "white", yaxt = "n")
